...
def test_gate_fails_without_raw_files(tmp_path):
    status = evaluate_extraction_gate(raw_dir=tmp_path, mapping_path=tmp_path / "moesm_strict_mapping.json")
...
def test_write_extraction_status_mentions_pending_phrase(tmp_path):
    out = tmp_path / "EXTRACTION_STATUS.md"
    write_extraction_status(raw_dir=tmp_path, out_path=out)
    text = out.read_text(encoding="utf-8")
    assert "raw MOESM verification pending" in text
