...
5. Maintain deterministic mapping in `data/raw/moesm_strict_mapping.json` and pass via `--mapping` when needed.

6. Treat `heuristic` mode as exploratory only; use `strict` mode for benchmark-grade outputs.
